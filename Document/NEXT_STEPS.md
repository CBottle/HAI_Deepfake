...
- ✅ `scripts/extract_frames.py` - 비디오 → 이미지 변환 (필요 시)
...
- ✅ Google Drive 통합
- ✅ 단계별 가이드 포함
- ✅ **이미지 데이터셋 지원** (`manjilkarki/deepfake-and-real-images`)
...
#### Step 2: Colab에서 데이터 다운로드 (10~30분)
```
1. Google Colab 접속
...
4. 셀 순서대로 실행
   - Step 1~5 실행
   - Deepfake and Real Images 데이터셋 다운로드
```
...
2. real/fake 폴더로 정리
   - train_data/real/ 과 train_data/fake/ 에 이미지 복사
...
**예상 소요 시간**: 총 30분~1시간
**필요한 것**: Kaggle 계정, Google Drive 5GB+

**✅ 이미지 데이터셋이라 프레임 추출 불필요!**
...
- [ ] Deepfake and Real Images 다운로드 (Step 5)
- [ ] 데이터 폴더 정리 (Step 6) - real/fake 구조로
- [ ] 소규모 데이터셋 생성 (Step 7)
- [ ] 데이터 검증 (Step 8)
